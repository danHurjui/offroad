 ...
 */
export type VehicleAccess = 'owner' | 'collaborator'
...
/** What a role in the vehicle's organisation grants on that vehicle. */
export function accessForRole(role: OrgRole): VehicleAccess {
  return MANAGING_ROLES.includes(role) ? 'owner' : 'collaborator'
}
 ...
 * FLEET_MANAGER are `owner`; MECHANIC and DRIVER are `collaborator`. An
...
export async function vehicleAccessFor(
  ...
  if (vehicle.organizationId) {
    ...
    if (membership) return accessForRole(membership.role)
...
export async function listAccessibleVehicles(userId: string): Promise<AccessibleVehicle[]> {
  const [owned, company, collaborating] = await Promise.all([
    ...
    prisma.vehicle.findMany({
      ...
      include: { organization: { select: { members: { where: { userId }, select: { role: true } } } } },
  ...
  for (const { organization, ...v } of company) {
    const role = organization?.members[0]?.role
    if (role) add(v, accessForRole(role))
  }
