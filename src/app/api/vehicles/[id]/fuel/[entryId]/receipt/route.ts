...
async function load(vehicleId: string, entryId: string, userId: string) {
  ...
  if (!entry || entry.vehicleId !== vehicle.id) return { error: await apiError('notFound', 404) }
  if (vehicle.ownerId !== userId && entry.createdByUserId !== userId) return { error: await apiError('fuelOwnOnly', 403) }
