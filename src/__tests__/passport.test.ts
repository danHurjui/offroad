...
const pdfStrings: PdfPassportStrings = {
  ...
  missing: ['Accidents and damage: none recorded in RigLog.'],
  ...
  documents: [],
  tyresLine: null,
  accidentsTitle: 'Accidents and damage',
  accidentsNote: 'Only what the owner chose to record.',
  accidents: [],
  noAccidents: 'Accidents and damage: none recorded in RigLog.',
  footer: 'Generated by RigLog',
}

function accident(o: Partial<PassportInput['accidents'][number]> = {}): PassportInput['accidents'][number] {
  return {
    date: day('2025-06-01'), kind: 'COLLISION', description: 'Rear bumper, hit at a junction.', km: 120_000,
    insurance: 'RCA', repairedAt: day('2025-06-20'), repairCostRon: 2400, photoCount: 2,
    createdAt: day('2025-06-02'), updatedAt: day('2025-06-02'), ...o,
  }
}
...
describe('it never claims to be more than the owner’s records', () => {
  ...
  it('with no accident records, says none were recorded — never that there were none', () => {
    const p = buildPassport(input({ rows, readings: [{ id: 'r', km: 1, readAt: day('2025-01-01'), isOverride: false }] }))
    expect(p.absences.map((a) => a.key)).toContain('absence.noAccidentsRecorded')
    expect(p.accidents).toEqual([])
  })

  it('lists accident records oldest first, with when each was entered, and drops the absence', () => {
    const p = buildPassport(input({ accidents: [accident({ date: day('2025-06-01') }), accident({ date: day('2024-03-02'), createdAt: day('2026-09-01'), updatedAt: day('2026-09-10') })] }))
    expect(p.absences.map((a) => a.key)).not.toContain('absence.noAccidentsRecorded')
    expect(p.accidents.map((a) => a.date)).toEqual([day('2024-03-02'), day('2025-06-01')])
    expect(p.accidents[0]).toMatchObject({ recordedAt: day('2026-09-01'), changedAt: day('2026-09-10'), photoCount: 2, repairCost: 2400 })
    expect(p.accidents[1].changedAt).toBeNull()
  })

  it('a repair cost is shown only when costs are on for the link', () => {
    const p = buildPassport(input({ options: { showPlate: false, showVin: false, showCosts: false }, accidents: [accident()] }))
    expect(p.accidents[0].repairCost).toBeNull()
  })

  it('the PDF has an accidents section, tagged, with the absence line when there are none', () => {
    const doc = buildPassportDocDefinition(buildPassport(input()), pdfStrings)
    const text = JSON.stringify(doc.content)
    expect(text).toContain('Accidents and damage')
    expect(text).toContain('none recorded in RigLog')
  })

  it('the page renders accident photos as a count, never as images', () => {
    const page = read('src/components/PassportDocument.tsx')
    const section = page.slice(page.indexOf('passport-accidents'), page.indexOf('passport-photos'))
    expect(section).toContain("t('accidentPhotos'")
