    ...
    <div>
      ...
      {comments.length > 0 && (
        <ul className="mb-5 space-y-3">
          {comments.map((c) => (
            <li key={c.id} className={`card p-4 ${c.isStaff ? 'border-brand-200 bg-brand-50' : ''}`}>
              <div className="mb-1 flex flex-wrap items-center gap-2">
                <span className="text-sm font-medium text-ink">{c.authorName}</span>
                {c.isStaff && <span className="badge bg-brand-100 text-brand-700">RigLog</span>}
      ...
      {signedIn ? (
        ...
        <p className="card p-4 text-sm text-ink-muted">
          <a href={`/login?callbackUrl=${encodeURIComponent(`/tickets/${ticketId}`)}`} className="text-brand-600 hover:underline">
            Log in
          </a>{' '}
          to comment or vote.
        </p>
